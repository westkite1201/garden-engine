...
꿀벌이 날아다니며 셀에 물을 주면, 기여 수에 따라 새싹 → 잎 → 봉오리 → 만개 꽃과 나무로 자라납니다.

<p align="center">
  <img src="assets/preview.svg" alt="꽃과 나무가 자란 정원 확대 예시 — 샘플 데이터" width="800"/>
</p>

위 확대 예시는 샘플 데이터입니다. 내 실제 기여 정원은 이 README 맨 위에서 확인할 수 있습니다.

## How it works

```
GitHub GraphQL API  →  Contribution Grid  →  Simulation Engine  →  Animated SVG
   (contributions)     (weekly cells)        (bee flights)       (CSS keyframes)
```

1. **Fetch** — GitHub GraphQL API에서 최근 1년 기여 데이터를 가져옵니다
2. **Map** — 커밋 수를 사분위수 기반으로 0~4 레벨로 매핑합니다
3. **Simulate** — 꿀벌이 정원 곳곳에서 출발해 가까운 미방문 셀로 날아가 물을 줍니다. 비행 경로는 교차할 수 있으며, 각 셀은 한 번만 물을 받습니다
4. **Render** — 5개 레이어(울타리, 흙, 성장, 이펙트, 액터)를 합성합니다

### 벌은 몇 마리 나오나요?

최근 1년의 **총 기여 수**에 따라 기본 벌 수가 달라집니다.
GitHub contribution 데이터에는 커밋 외에 PR·이슈 등의 활동도 포함됩니다.
...
활동한 날짜가 많으면 **활성 날짜 30개당 벌 1마리(올림)** 기준도 함께 적용하고,
두 기준 중 큰 값을 사용합니다. 최종 상한은 **12마리**, 그리고 활성 날짜 수입니다.
기여가 없으면 벌이 나오지 않고, 하루에 기여가 몰렸다면 벌도 1마리만 나옵니다.
예를 들어 61일 동안 기여 200개라면 벌 3마리, 200일 동안 기여 1,500개라면 벌 7마리입니다.

벌은 서로 다른 활성 셀에서 짧은 간격으로 등장하며, 물을 주는 동안 제자리에 머뭅니다.
날개를 작게 퍼덕이며 이동 방향으로 몸을 돌리고, 물을 주는 칸에는 작은 반짝임 3개가 나타납니다.
모든 셀의 성장이 끝나면 꽃과 셀 전체에 개화 파동이 퍼집니다.
...
기여 활동에 따라 새싹 → 잎 → 봉오리 → 만개 꽃과 나무로 성장합니다.
꿀벌이 물을 준 뒤 식물이 나타나고, 모든 셀이 자라면 개화 파동이 퍼집니다.

| Level | 기준 | 식물 | 바닥 색 |
|-------|------|------|---------|
| 0 | 기여 없음 | 흙과 작은 자연 장식 | `#2a1f14` |
| 1 | 하위 25% | 새싹 | `#0e4429` |
| 2 | 25~50% | 잎 | `#006d32` |
| 3 | 50~75% | 봉오리 | `#26a641` |
| 4 | 상위 25% | 만개 꽃·해바라기·참나무·침엽수 | `#39d353` |

꽃은 셀 위치에 따라 핑크·라벤더·코랄·하늘색·주황색·자홍색 중 하나로 결정됩니다.
레벨 4의 약 20%는 나무로 표현됩니다. 같은 데이터에서는 같은 식물이 유지됩니다.

기존 흙·잔디 색, 울타리와 칸 간격을 유지합니다.
꽃잎의 음영과 나무 줄기를 조금 더 또렷하게 다듬었으며, 식물은 기존 10×10px 칸 안에 들어갑니다.
...
| `rules` | 체류 시간, `actorCount(activeCells, totalContributions)`, 셀 이동 속도 |
